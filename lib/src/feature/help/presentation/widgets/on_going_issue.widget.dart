...
class OnGoingIssue extends StatefulWidget {
  const OnGoingIssue({super.key});

  @override
  State<OnGoingIssue> createState() => _OnGoingIssueState();
}

class _OnGoingIssueState extends State<OnGoingIssue> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<SupportCenterBloc, SupportCenterState>(
      ...
      builder: (context, state) {
        if (state is SupportCenterLoading) {
          return AppOverlayLoadingWidget(height: height * 0.6, width: width);
        } else if (state is SupportCenterThreadsData) {
          final data = state.onGoingIssues;
          return Column(
            children: [
